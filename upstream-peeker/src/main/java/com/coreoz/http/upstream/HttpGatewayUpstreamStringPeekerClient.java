...
public class HttpGatewayUpstreamStringPeekerClient {
    ...
    public static final BiFunction<Http.Request, byte[], String> DOWNSTREAM_STRING_PEEKER = (downstreamRequest, bytesPeeked) -> new String(
        bytesPeeked,
        HttpCharsetParser.parseEncodingFromHttpContentType(
            downstreamRequest.contentType().orElse(null)
        )
    );
    public static final BiFunction<HttpGatewayUpstreamResponse, byte[], String> UPSTREAM_STRING_PEEKER = (upstreamRequest, bytesPeeked) -> new String(
        bytesPeeked,
    ...
    public HttpGatewayUpstreamStringPeekerClient(HttpGatewayStringStreamPeekingConfiguration defaultConfiguration) {
        this.bytesPeekerClient = new HttpGatewayUpstreamBytesPeekerClient<>(new HttpGatewayBytesStreamPeekingConfiguration<>(
